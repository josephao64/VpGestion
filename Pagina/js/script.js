...
function toggleFields(letterType) {
...
// Función para manejar la selección de tipo de carta
function selectLetterType(event) {
    const button = event.currentTarget;
    const letterType = button.getAttribute('data-letter-type');

    // Mostrar el formulario
    document.getElementById("formContainer").style.display = "block";

    // Mostrar u ocultar campos según el tipo de carta
    toggleFields(letterType);
}

// Agregar eventos a los botones de tipo de carta
document.addEventListener('DOMContentLoaded', () => {
    const letterButtons = document.querySelectorAll('.letter-button');
    letterButtons.forEach(button => {
        button.addEventListener('click', selectLetterType);
    });
});

// Función para generar la carta en HTML
function generateLetter() {
    const letterTypeButtons = document.querySelectorAll('.letter-button');
    let selectedLetterType = "";

    // Obtener el tipo de carta seleccionado
    letterTypeButtons.forEach(button => {
        if (button.classList.contains('active')) {
            selectedLetterType = button.getAttribute('data-letter-type');
        }
    });

    // Si ningún tipo está activo, obtenerlo de los campos visibles
    if (!selectedLetterType) {
        const letterTypeSelection = document.getElementById("letterTypeSelection");
        if (letterTypeSelection) {
            // Por seguridad, evitar errores si no se selecciona ningún botón
            alert("Por favor, seleccione el tipo de carta.");
            return;
        }
    }
    ...
    switch (company) {
        case "american_pizza":
            headerContent = `<header>
                                <img src="images/2.png" alt="Logo American Pizza" width="70" height="25">
                             </header>`;
            break;
        case "brenda_elizabeth":
            headerContent = `<header>
                                <img src="images/1.png" alt="Logo Brenda Elizabeth Hernández" width="70" height="25">
                             </header>`;
            break;
        case "corporacion_alimentos":
            headerContent = `<header>
                                <img src="images/1.png" alt="Logo Corporación de Alimentos S.A. American Pizza" width="70" height="25">
                             </header>`;
            break;
        default:
            headerContent = `<header>
...
async function exportPDF() {
    ...
    // Obtener el tipo de carta desde el contenido generado
    const letterTypeContent = document.getElementById("letterContent").querySelector("h1").textContent;
    let letterType = "";
    if (letterTypeContent.includes("Vacaciones")) {
        letterType = "vacaciones";
    } else if (letterTypeContent.includes("Despido")) {
        letterType = "despido";
    }
    ...
    const addImageToPDF = async (imgElementId, x, yPos, width, height) => {
        return new Promise((resolve, reject) => {
            const img = document.querySelector(`#letterContent img`);
            if (!img) {
                resolve();
                return;
            }
    ...
    if (company) {
        switch (company) {
            case "american_pizza":
                ...
                doc.setFontSize(12);
                doc.text("Dirección: Dirección de American Pizza | Tel: (123) 456-7890", 105, y, { align: "center" });
                break;
            case "brenda_elizabeth":
                await addImageToPDF('logo1', marginLeft, y, 70, 25); // Tamaño ajustado: ancho=70, alto=25
                y += 20;
                doc.setFont("Helvetica", "bold");
                doc.setFontSize(16);
                ...
                doc.setFont("Helvetica", "normal");
                doc.setFontSize(12);
                doc.text("Dirección: Dirección de Brenda Elizabeth Hernández | Tel: (123) 456-7890", 105, y, { align: "center" });
                break;
            case "corporacion_alimentos":
                await addImageToPDF('logo1', marginLeft, y, 70, 25); // Tamaño ajustado: ancho=70, alto=25
                y += 20;
                ...
                doc.setFont("Helvetica", "normal");
                doc.setFontSize(12);
                doc.text("Dirección: Dirección de Corporación de Alimentos S.A. | Tel: (123) 456-7890", 105, y, { align: "center" });
                break;
            default:
                doc.setFont("Helvetica", "bold");
                doc.setFontSize(16);
